                    ...
                    <div class="portlet-body">
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                      <button type="button" name="create_record" id="create_record" class="btn sbold green"> <i class="fa fa-plus"></i> اضافة عميل جديد</button>
                                    </div>
                                </div> 
                            </div>
                        </div>
        ...
        <div class="loading hidden">Loading&#8230;</div>
         ...
         <form method="post" id="sample_form" class="form-horizontal" enctype="multipart/form-data">
          ...
          <div class="form-group">
           ...
           @for ($i = 0; $i < 2; $i++)
           <div class="form-group">
            <label class="control-label col-md-2" >رقم التليفون {{$i+1}} </label>
            <div class="col-md-10">
             <input type="text" name="phone[]" id="phone{{$i}}" class="form-control" />
            </div>
           </div>
           @endfor
           <div class="form-group">
...
$(document).ready(function(){
 ...
 $('#sample').DataTable({
  ...
  $('#create_record').click(function(){
    $('.modal-title').text("اضف عميل جديدة");
...
$('#sample_form').on('submit', function(event){
  ...
  {
   $.ajax({
    url:"{{ route('dashboard.clients.update') }}",
    method:"POST",
    data:new FormData(this),
    contentType: false,
    cache: false,
...
$(document).on('click', '#bulk_delete', function(){
      ...
      if (result.value) {
          ...
          if(id.length > 0)
            {
                $.ajax({
                    url:"{{ route('dashboard.clients.destroy.all')}}",
                    method:"get",
                    data:{id:id},
                    beforeSend:function(){
                      $('.loading').removeClass('hidden');
