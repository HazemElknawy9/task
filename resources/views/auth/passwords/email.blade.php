    ...
    <body class=" login">
        ...
        <div style="border-radius: 13px !important;" class="content">
            ...
            <form class="login-form" method="POST" action="{{ route('password.email') }}">
                ...
                <p> ادخل البريد الإلكتروني لإستراجاع كلمة المرور. </p>
                <div class="form-group">
                    <div class="input-icon">
                        <i class="fa fa-envelope"></i>
                        <input id="email" type="email" class="form-control placeholder-no-fix {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required  placeholder="البريد الإلكتروني" /> 
                        @if ($errors->has('email'))
