...
class ClientController extends Controller
{
    ...
    public function index()
    {
        if(request()->ajax())
        {
            return datatables()->of(Client::latest()->get())
                    ...
                    ->addColumn('add_order', function($data){
                         $button = '<a href="clients/'.$data->id.'/orders/create" class="btn btn-info">اضف طلب</a>';
                        return $button;
                    })
                    ...
                    ->rawColumns(['action','add_order','checkbox','active','inactive','user_active_or_not'])
                    ->make(true);
        }           
    ...
    public function destroyAll(Request $request)
    {
        ...
        $category = Client::whereIn('id', $category_id_array);
        if($category->delete())
        {
            return response()->json(['success' => 'تم حذف البيانات بنجاح']);
        }
    ...
    public function clientActive($status,$id)
    {
